...
def create_user():
    ...
    if not user_datastore.get_user('joao'):
        user_datastore.create_user(email='joao', password=flask_security.utils.hash_password('123'))
        user_datastore.add_role_to_user('joao', 'owner')
    if not user_datastore.get_user('maria'):
        user_datastore.create_user(email='maria', password=flask_security.utils.hash_password('123'))
